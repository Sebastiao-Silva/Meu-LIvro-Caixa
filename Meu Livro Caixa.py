...
if not st.session_state.logado:
    ...
    cliente_final = None
    cat_final = None
    pode_lancar = False # Trava de segurança inicial
    ...
    with tab_a:
        ...
        if sel_a != "-- Selecionar --":
            cliente_final = sel_a
            cat_final = "Aluno"
            pode_lancar = True # Libera apenas para esta aba
    ...
    with tab_r:
        # Aqui pode_lancar continua False, escondendo tudo que é de ação
        devedores_lista = []
        total_geral = 0
    ...
    # --- ÁREA DE LANÇAMENTO (TOTALMENTE TRAVADA SE ESTIVER NO RELATÓRIO) ---
    if cliente_final and cat_final and pode_lancar:
        ...
        if 'op' in st.session_state:
            with st.form("l_form"):
                ...
                if 'val_in' not in st.session_state: st.session_state.val_in = 0.0
                vf = st.number_input("Valor R$", min_value=0.0, value=st.session_state.val_in)
                cv1, cv2, cv3 = st.columns(3)
                with cv1: 
                    if st.form_submit_button("R$ 6,00"): 
                        st.session_state.val_in = 6.0
                        st.rerun()
                with cv2:
                    if st.form_submit_button("R$ 7,00"):
                        st.session_state.val_in = 7.0
                        st.rerun()
                with cv3:
                    if st.form_submit_button("R$ 8,00"):
                        st.session_state.val_in = 8.0
                        st.rerun()
